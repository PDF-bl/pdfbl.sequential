...
def test_copy():
    ...
    assert nx.utils.graphs_equal(dag_from_str, copied_dag)
    ...
    for node_id in dag_from_str.nodes():
        assert node_id in copied_dag.nodes
        assert copied_dag.nodes[node_id]["payload"] is None
    assert nx.is_isomorphic(dag_from_str, copied_dag)
    ...
    for node_id in dag_from_str.nodes():
        assert node_id not in copied_dag.nodes
