...
class PDFSequentialFit:
    ...
    def _check_for_new_profiles(self):
        ...
        if self.profiles_known != files[: len(self.profiles_known)]:
            raise ValueError(
                ...
                "Please ensure newer profiles are have higher "
                "indices."
            )
    ...
    def _launch(self):
        for profile in self.profiles_running:
            ...
            print(f"Finished {len(self.profiles_finished)+1} fit tasks.")
        self.profiles_running = []
    ...
    def watch(self, pname, when, **kwargs):
        """Monitor the results generated during the fit process.
