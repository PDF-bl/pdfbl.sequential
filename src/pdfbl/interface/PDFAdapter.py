...
class PDFAdapter:
    ...
    def delVar(self, var_name):
        ...
        variables will be refined in `all` by default. Use this method to
        remove a variable from the recipe if it should not be refined.
    ...
    def action_func_factory(self, action_names):
        ...
        def action_func():
            # Currently only allow 'free' variables due to the
    ...
    def _residual(self, p=[]):
        """Residual function modified from ``FitRecipe`` in order to
        capture the intermediate results, the snapshots, during the
        iterations."""
    ...
    def clone(self):
        ...
        adapter.snapshots = self.snapshots.copy()
        return adapter
